...
export type ProcessDataResult = { value: number };
/**
 * Returns the parsed numeric `value`, or null as a sentinel for every invalid
 * input (unparseable JSON, missing/non-numeric value, NaN/Infinity). Using null
 * instead of 0 lets callers distinguish a failure from a legitimate zero.
 */
export function processData(input: string): ProcessDataResult | null {
  try {
    ...
    [-20000, -20000 * EXTREME_MULTIPLIER],
...
describe("processData", () => {
  ...
  test('valid JSON value 0 is preserved (distinct from failure)', () => {
  ...
  test.each([
    ...
    ['{"value":null}'],
    ['{"value":1e999}'],
  ])('invalid input %s returns null', (input) => {
  ...
  test('whitespace-padded JSON is parsed', () => {
    expect(processData(WHITESPACE_PADDING + '{"value": ' + SAMPLE_VALUE + '} ' + WHITESPACE_PADDING)).toEqual({ value: SAMPLE_VALUE });
  });
