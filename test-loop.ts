...
describe("calculate", () => {
  test.each([
    ...
    [EXTREME_THRESHOLD * 2, EXTREME_THRESHOLD * 2 * EXTREME_MULTIPLIER],
    [NaN, NaN],
    [Infinity, Infinity],
    [-Infinity, -Infinity],
...
describe("processData", () => {
  ...
  test(`inputs above 2^${MAX_SAFE_INTEGER_BITS} lose integer precision (documented limitation)`, () => {
    const input = 9007199254740993n;
    const expected = input * BigInt(EXTREME_MULTIPLIER);
    expect(BigInt(calculate(Number(input)))).not.toBe(expected);
    expect(Number.isSafeInteger(calculate(Number(input)))).toBe(false);
  });
  test.each([
    ["not-json"],
    ["[1,2,3]"],
