...
function parseEslintResults(results: { filePath: string; messages: { line: number; severity: number; message: string; ruleId: string | null }[] }[]): Finding[] {
...
const eslint: LinterTool = {
  ...
  run(root: string, extraArgs: string[]): Finding[] {
    try {
      ...
      if (!out.trim()) return [];
      let results: { filePath: string; messages: { line: number; severity: number; message: string; ruleId: string | null }[] }[] = [];
      try {
        results = JSON.parse(out);
      } catch (e) {
        logger.warn(`eslint JSON parse failed: ${e}`);
        return [];
      }
...
const biome: LinterTool = {
  ...
  run(root: string, extraArgs: string[]): Finding[] {
    try {
      ...
      if (!out.trim()) return [];
      let parsed: { diagnostics: { location: { path: { file: string }; span: { start: { line: number } } | null }; severity: string; message: { text: string }; category: string }[] } = { diagnostics: [] };
      try {
        parsed = JSON.parse(out);
      } catch (e) {
        logger.warn(`biome JSON parse failed: ${e}`);
        return [];
      }
  ...
  run(root: string, extraArgs: string[]): Finding[] {
    try {
      ...
      try {
        ...
        logger.warn(`pylint JSON parse failed: ${e}`);
        return [];
      }
