...
function parseAiResponse(
  ...
  try {
    const indices = filterValidIndices(JSON.parse(content), maxIndex);
  ...
  if (extracted) {
    try {
      const indices = filterValidIndices(JSON.parse(extracted[0]), maxIndex);
      if (indices !== null) return indices;
    } catch {
      // fall through
    }
