...
export function parseDismissArgs(dismissArgs: string[]): DismissArgs {
  ...
  return { reason: "dismissed by user", error: "Cannot --rule or --file." };
}
...
export function runSetup(force: boolean): void {
  ...
  writeWorkflowFile(workflowPath, WORKFLOW_CONTENT, "codesentinel.yml", ".github/workflows/codesentinel.yml");
  writeWorkflowFile(buildWorkflowPath, BUILD_WORKFLOW_CONTENT, "codesentinel-build.yml", ".github/workflows/codesentinel-build.yml");
  process.stdout.write(`\n`);

  printNextSteps();
}

function writeWorkflowFile(path: string, content: string, backupLabel: string, createdLabel: string): void {
  if (existsSync(path)) {
    writeFileSync(`${path}.bak`, readFileSync(path), "utf8");
    process.stdout.write(`Backed up existing ${backupLabel} to ${backupLabel}.bak\n`);
  }
  writeFileSync(path, content, "utf8");
  process.stdout.write(`✅ Created ${createdLabel}\n`);
}
