...
"            }",
  "",
  "  slash-command:",
  "    if: github.event_name === 'issue_comment' && github.event.action === 'created' && contains('OWNER MEMBER COLLABORATOR', github.event.comment.author_association)",
  "    runs-on: ubuntu-latest",
  "    steps:",
  "      - name: Is PR comment?",
  "        id: is_pr",
...
'          CODESENTINEL_GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN }}',
  ...
  '            git commit -m "fix: auto-fix [skip ci]"',
  ...
  "            git pull --rebase --autostash origin ${{ github.ref_name }} 2>&1 || { echo \"❌ Rebase conflict — aborting fix loop\"; git rebase --abort 2>/dev/null || true; echo \"::endgroup::\"; exit 1; }",
