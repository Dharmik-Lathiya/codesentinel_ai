...
function writeWorkflow(workflowDir: string, fileName: string, label: string, content: string): void {
  const path = join(workflowDir, fileName);
  if (existsSync(path)) {
    writeFileSync(`${path}.bak`, readFileSync(path), "utf8");
    process.stdout.write(`Backed up existing ${label} to ${fileName}.bak\n`);
  }
  writeFileSync(path, content, "utf8");
  process.stdout.write(`\n✅ Created .github/workflows/${fileName}\n`);
}
...
export function runSetup(force: boolean): void {
  ...
  writeWorkflow(workflowDir, "codesentinel.yml", "workflow", WORKFLOW_CONTENT);
  writeWorkflow(workflowDir, "codesentinel-build.yml", "build-fix workflow", BUILD_WORKFLOW_CONTENT);
  process.stdout.write("\n");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`CodeSentinel [${runMode}] run failed: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exitCode = 1;
    return;
  }
