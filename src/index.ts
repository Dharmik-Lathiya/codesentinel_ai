...
"          script: |",
"            const fs = require('fs');",
  "            let out = ''; try { out = fs.readFileSync('/tmp/cs-out.txt','utf8'); } catch {}; out = out.replace(/```/g, '~~~');",
...
export const BUILD_WORKFLOW_CONTENT = [
  ...
  "jobs:",
  "  build-fix:",
  "    if: ${{ github.event_name === 'push' && !startsWith(github.actor, 'codesentinel') && !contains(github.event.head_commit.message, '[skip ci]') }}",
  "    steps:",
  "      - uses: actions/checkout@v4",
  "        with:",
  "          fetch-depth: 0",
...
'          CODESENTINEL_GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN }}',
  ...
  "            git commit -m \"Build fix via CodeSentinel [skip ci]\"",
  ...
  '              echo "✅ Fix pushed to ${{ github.ref_name }}"',
  "            fi",
  "            git remote set-url origin https://github.com/${{ github.repository }}.git 2>&1 || true",
  "          done",
  "",
