...
export function parseDismissArgs(dismissArgs: string[]): DismissArgs {
  ...
  if (hasRule) {
    return parseDismissByRule(dismissArgs, ruleIdx);
  }

  if (hasFile) {
    return parseDismissByFile(dismissArgs, fileIdx);
  }

  return { reason: "dismissed by user", error: "Missing --rule or --file." };
}

function parseDismissByRule(dismissArgs: string[], ruleIdx: number): DismissArgs {
...
function parseDismissByFile(dismissArgs: string[], fileIdx: number): DismissArgs {
...
function writeWorkflowWithBackup(
  filePath: string,
  content: string,
  backupLabel: string,
  ...
  if (existsSync(filePath)) {
    ...
    process.stdout.write(`Backed up existing ${backupLabel}\n`);
  }
  mkdirSync(dirname(filePath), { recursive: true });
  writeFileSync(filePath, content, "utf8");
  process.stdout.write(createdMessage);
}
...
export function runSetup(force: boolean): void {
  ...
  writeWorkflowWithBackup(
    workflowPath,
    WORKFLOW_CONTENT,
    "workflow to codesentinel.yml.bak",
    "\n✅ Created .github/workflows/codesentinel.yml\n",
  );
  writeWorkflowWithBackup(
    buildWorkflowPath,
    BUILD_WORKFLOW_CONTENT,
    "build-fix workflow to codesentinel-build.yml.bak",
    "✅ Created .github/workflows/codesentinel-build.yml\n\n",
  );
