...
function backupWorkflow(workflowPath: string, backupName: string): void {
  if (existsSync(workflowPath)) {
    writeFileSync(`${workflowPath}.bak`, readFileSync(workflowPath), "utf8");
    process.stdout.write(`Backed up existing workflow to ${backupName}.bak\n`);
  }
}

function writeWorkflowFile(workflowPath: string, content: string, backupName: string): void {
  backupWorkflow(workflowPath, backupName);
...
export function runSetup(force: boolean): void {
  ...
  writeWorkflowFile(workflowPath, WORKFLOW_CONTENT, "codesentinel.yml");
  process.stdout.write("\n✅ Created .github/workflows/codesentinel.yml\n");
  writeWorkflowFile(buildWorkflowPath, BUILD_WORKFLOW_CONTENT, "codesentinel-build.yml");
