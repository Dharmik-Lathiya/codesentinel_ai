...
function writeWorkflowFile(filePath: string, content: string, backupLabel: string, backupName: string): void {
  if (existsSync(filePath)) {
    writeFileSync(`${filePath}.bak`, readFileSync(filePath), "utf8");
    process.stdout.write(`Backed up existing ${backupLabel} to ${backupName}.bak\n`);
  }
  writeFileSync(filePath, content, "utf8");
}

function printSetupNotes(): void {
...
export function runSetup(force: boolean): void {
  ...
  writeWorkflowFile(workflowPath, WORKFLOW_CONTENT, "workflow", "codesentinel.yml");
  writeWorkflowFile(buildWorkflowPath, BUILD_WORKFLOW_CONTENT, "build-fix workflow", "codesentinel-build.yml");
