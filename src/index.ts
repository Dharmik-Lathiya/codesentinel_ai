...
"            }",
  ...
  "                core.setFailed('You need write permission to run CodeSentinel commands');",
  "              }",
  "            } catch (err) {",
  "              core.setFailed(err.message);",
  "            }",
...
"            const match = body.match(/^\\/(review|fix|audit|score|testgen|gate|describe|plan|improve|ask)\\b/i);",
...
'          CODESENTINEL_GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN }}',
  ...
  "              git push origin HEAD:${{ github.ref_name }} 2>&1",
  "              if [ $? -ne 0 ]; then",
  '                echo "❌ Push failed after rebase — exiting fix loop"',
  '                echo "::endgroup::"',
  "                exit 1",
  "              fi",
