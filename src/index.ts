...
export function parseDismissArgs(dismissArgs: string[]): DismissArgs {
  ...
  if (hasRule && hasFile) {
    return { reason: "Options --rule and --file are mutually exclusive.", error: "Options --rule and --file are mutually exclusive." };
  }
...
'          CODESENTINEL_GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN }}',
  ...
  "            GIT_PUSH_TOKEN=\"${CODESENTINEL_GITHUB_TOKEN:-${GITHUB_TOKEN}}\"",
  "            echo \"https://x-access-token:${GIT_PUSH_TOKEN}@github.com\" > \"$HOME/.git-credentials\"",
