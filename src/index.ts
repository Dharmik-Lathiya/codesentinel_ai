...
"            }",
  ...
  "            const fs = require('fs');",
  "            let out = ''; try { out = fs.readFileSync('/tmp/cs-out.txt','utf8'); } catch {}",
  "            const body = '### CodeSentinel \\u2014 Plan Failed\\n\\n' + (out || 'An error occurred while generating the implementation plan. Please check the workflow logs.') + '\\n\\nYou can retry by commenting `/plan` again.';",
  "            try {",
  "              await github.rest.issues.updateComment({",
  "                owner: context.repo.owner, repo: context.repo.repo,",
  "                comment_id: ${{ steps.loading.outputs.comment_id }},",
  ...
  "",
  "  slash-command:",
  "    if: github.event_name == 'issue_comment' && github.event.action == 'created' && (startsWith(github.event.comment.body, '/review') || startsWith(github.event.comment.body, '/fix') || startsWith(github.event.comment.body, '/audit') || startsWith(github.event.comment.body, '/score') || startsWith(github.event.comment.body, '/testgen') || startsWith(github.event.comment.body, '/gate') || startsWith(github.event.comment.body, '/deadcode') || startsWith(github.event.comment.body, '/describe') || startsWith(github.event.comment.body, '/plan') || startsWith(github.event.comment.body, '/ask'))",
  "    runs-on: ubuntu-latest",
  "    steps:",
  "      - name: Is PR comment?",
  "        id: is_pr",
  ...
  "            if (!match) { console.log('No valid command — ignoring comment'); return; }",
  "            const mode = match[1].toLowerCase();",
  "            const isAsk = mode === 'ask';",
  "            const chatMode = isAsk ? 'chat' : mode;",
  "            const question = isAsk ? body.replace(/^\\/ask\\s*/i, '').trim() : '';",
  "            core.setOutput('mode', chatMode);",
  ...
  "                });",
  "              } catch (err) {",
  "                core.setFailed('@' + actor + ' is not authorized to run CodeSentinel commands. Only the repository owner and collaborators can trigger them.');",
  "              }",
  "            }",
...
const BUILD_WORKFLOW_CONTENT = [
  ...
  "  workflow_dispatch:",
  "    inputs:",
  "      max-build-fix-iterations:",
  "        description: 'Maximum auto-fix iterations before giving up'",
  "        required: false",
  "        default: '5'",
  "jobs:",
  "  build-fix:",
...
'          CODESENTINEL_GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN }}',
'          MAX_BUILD_FIX_ITERATIONS: ${{ inputs.max-build-fix-iterations }}',
  "",
  "          MAX_ITER=\"${MAX_BUILD_FIX_ITERATIONS:-5}\"",
  ...
  "            fi",
  "            if [ ! -d \"/tmp/codesentinel\" ]; then",
  '              echo "Cloning CodeSentinel..."',
  "              git clone --depth 1 https://github.com/Dharmik-Lathiya/CodeSentinel_AI.git /tmp/codesentinel",
  '              cd /tmp/codesentinel && npm ci --omit=dev --ignore-scripts --no-audit --no-fund 2>&1 && cd ..',
  "            fi",
  "",
  '              node /tmp/codesentinel/dist/index.js fix --auto-fix 2>&1 || echo "Fix step completed with warnings"',
  "",
  "            git add -u",
  ...
  "            git pull --rebase origin ${{ github.ref_name }} 2>&1 || { echo \"❌ Rebase failed (conflicts). Aborting to avoid losing the fix.\"; exit 1; }",
  ...
  "            else",
  '              echo "❌ Failed to push to ${{ github.ref_name }}"',
  "              exit 1",
  "            fi",
  "          done",
  "",
  "            BASE64_PUSH_TOKEN=$(printf '%s' \"x-access-token:${GIT_PUSH_TOKEN}\" | base64)",
  "            git -c http.extraheader=\"AUTHORIZATION: basic ${BASE64_PUSH_TOKEN}\" push origin HEAD:${{ github.ref_name }} 2>&1",
  "          echo \"::endgroup::\"",
  "          exit 1",
  "",
  "      - name: Notify failure",
