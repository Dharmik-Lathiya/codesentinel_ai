...
function backupWorkflow(workflowPath: string, label: string): void {
  if (existsSync(workflowPath)) {
    writeFileSync(`${workflowPath}.bak`, readFileSync(workflowPath), "utf8");
    process.stdout.write(`Backed up existing workflow to ${label}\n`);
  }
}

function writeSetupGuide(): void {
...
export function runSetup(force: boolean): void {
  ...
  backupWorkflow(workflowPath, "codesentinel.yml.bak");
  mkdirSync(workflowDir, { recursive: true });
  writeFileSync(workflowPath, WORKFLOW_CONTENT, "utf8");
  process.stdout.write(`\n✅ Created .github/workflows/codesentinel.yml\n`);

  backupWorkflow(buildWorkflowPath, "codesentinel-build.yml.bak");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`[codesentinel:error] Mode ${runMode} failed: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exitCode = 1;
    return;
  }
