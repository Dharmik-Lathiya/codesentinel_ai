...
function backupWorkflow(path: string, label: string): void {
  if (existsSync(path)) {
    writeFileSync(`${path}.bak`, readFileSync(path), "utf8");
    process.stdout.write(`Backed up existing ${label} to ${label}.bak\n`);
  }
}
