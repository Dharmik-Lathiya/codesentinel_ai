...
"            }",
  ...
  "            if (!match) { core.setOutput('mode', 'none'); return; }",
...
'          MAX_ITERATIONS: 5',
  ...
  "            GIT_PUSH_TOKEN=\"${CODESENTINEL_GITHUB_TOKEN:-${GITHUB_TOKEN}}\"",
  "            if git push \"https://x-access-token:${GIT_PUSH_TOKEN}@github.com/${{ github.repository }}.git\" HEAD:${{ github.ref_name }} >/tmp/push.log 2>&1; then",
  '              echo "✅ Fix pushed to ${{ github.ref_name }}"',
  "            else",
  "              sed -i \"s|${GIT_PUSH_TOKEN}|[REDACTED]|g\" /tmp/push.log",
  "              cat /tmp/push.log",
  '              echo "❌ Failed to push fix to ${{ github.ref_name }}"',
  "              echo \"::endgroup::\"",
  "              exit 1",
  "            fi",
