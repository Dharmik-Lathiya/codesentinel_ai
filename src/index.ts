...
export function runSetup(force: boolean): void {
  ...
  backupFileIfExists(workflowPath, "workflow to codesentinel.yml.bak");
  mkdirSync(workflowDir, { recursive: true });
  writeFileSync(workflowPath, WORKFLOW_CONTENT, "utf8");
  process.stdout.write(`\n✅ Created .github/workflows/codesentinel.yml\n`);

  backupFileIfExists(buildWorkflowPath, "build-fix workflow to codesentinel-build.yml.bak");
...
function backupFileIfExists(path: string, label: string): void {
  if (existsSync(path)) {
    writeFileSync(`${path}.bak`, readFileSync(path), "utf8");
    process.stdout.write(`Backed up existing ${label}\n`);
  }
}
