...
"            }",
  "",
  "      - name: Sanitize issue title/body",
  "        id: issue_info",
  "        uses: actions/github-script@v7",
  "        with:",
  "          script: |",
  "            const sanitize = (s) => String(s || '').replace(/[\\u0000-\\u0008\\u000B\\u000C\\u000E-\\u001F\\u007F]/g, '').replace(/\\r?\\n/g, ' ').trim();",
  "            core.setOutput('title', sanitize(context.payload.issue?.title));",
  "            core.setOutput('body', sanitize(context.payload.issue?.body).slice(0, ${MAX_ISSUE_BODY_LENGTH}));",
  "",
...
"            const fs = require('fs');",
"            let out = '';",
"            if (fs.existsSync('/tmp/cs-out.txt')) { try { out = fs.readFileSync('/tmp/cs-out.txt','utf8'); } catch {} }",
"            if (!out.trim()) { out = '⚠️ CodeSentinel finished but produced no output.'; }",
...
"            const question = rawMode === 'ask' ? body.replace(/^\\/ask\\s*/i, '').trim() : '';",
  ...
  "              });",
  "              const sanitize = (s) => String(s || '').replace(/[\\u0000-\\u0008\\u000B\\u000C\\u000E-\\u001F\\u007F]/g, '').replace(/\\r?\\n/g, ' ').trim();",
  "              core.setOutput('title', sanitize(issue.title));",
  "              core.setOutput('body', sanitize(issue.body).slice(0, ${MAX_ISSUE_BODY_LENGTH}));",
  "            } catch (err) { core.setFailed(err.message); }",
  "",
  "      # Uses pre-built composite action — no npm install + tsc build",
  "      # CODESENTINEL_GITHUB_TOKEN: optional PAT for git push (higher permissions)",
...
'          CODESENTINEL_GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN }}',
  ...
  "            git pull --rebase origin ${{ github.ref_name }} 2>&1 || { echo 'Rebase failed; aborting'; git rebase --abort; exit 1; }",
"            git remote set-url origin \"https://x-access-token:${GIT_PUSH_TOKEN}@github.com/${{ github.repository }}.git\" 2>&1",
"            FIX_BRANCH=\"codesentinel-build-fix-${{ github.run_id }}\"",
"            git checkout -b \"$FIX_BRANCH\"",
"            git push \"https://x-access-token:${GIT_PUSH_TOKEN}@github.com/${{ github.repository }}.git\" \"HEAD:$FIX_BRANCH\" 2>&1",
'            gh pr create --base "${{ github.ref_name }}" --head "$FIX_BRANCH" --title "CodeSentinel: auto-fix build errors" --body "Auto-generated fix by CodeSentinel. Please review before merging." 2>&1 || echo "Could not create PR (may already exist)"',
"            echo \"::endgroup::\"",
"            exit 0",
  "          done",
  "",
