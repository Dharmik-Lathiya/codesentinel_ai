...
function backupExisting(filePath: string, message: string): void {
...
export function runSetup(force: boolean): void {
  ...
  backupExisting(workflowPath, "Backed up existing workflow to codesentinel.yml.bak\n");
  mkdirSync(workflowDir, { recursive: true });
  writeFileSync(workflowPath, WORKFLOW_CONTENT, "utf8");
  process.stdout.write(`\n✅ Created .github/workflows/codesentinel.yml\n`);

  backupExisting(buildWorkflowPath, "Backed up existing build-fix workflow to codesentinel-build.yml.bak\n");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`[codesentinel:error] ${err instanceof Error ? err.message : String(err)}\n`);
    process.exitCode = 1;
    return;
  }
