...
"            }",
  ...
  "        with:",
  "          mode: plan",
  "      # Base64-encode issue title/body to prevent YAML/script injection",
  "      - name: Encode issue title/body",
  "        id: enc",
  "        uses: actions/github-script@v7",
  "        with:",
  "          script: |",
  "            const title = String(context.payload.issue?.title || '');",
  "            const body = String(context.payload.issue?.body || '').slice(0, " + `${MAX_ISSUE_BODY_LENGTH}` + ");",
  "            core.setOutput('title', Buffer.from(title).toString('base64'));",
  ...
  "        with:",
  "          mode: plan",
  "          issue_title: ${{ steps.enc.outputs.title }}",
  "          issue_body: ${{ steps.enc.outputs.body }}",
...
"            core.setOutput('question', question);",
  ...
  "        with:",
  "          script: |",
  "            core.setOutput('question', Buffer.from(question).toString('base64'));",
  ...
  "        id: cs",
  "              core.setOutput('title', Buffer.from(issue.title).toString('base64'));",
  "              core.setOutput('body', Buffer.from((issue.body || '').slice(0, " + `${MAX_ISSUE_BODY_LENGTH}` + ")).toString('base64'));",
  "          GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN || secrets.GITHUB_TOKEN }}",
  "        with:",
...
'          CODESENTINEL_GITHUB_TOKEN: ${{ secrets.CODESENTINEL_GITHUB_TOKEN }}',
  ...
  "          MAX_ITER=5  # Max fix loop iterations (build-fix)",
  "            if [ $FAILED -eq 0 ]; then",
