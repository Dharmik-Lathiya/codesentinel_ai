...
export class PluginManager {
  ...
  private async loadAndInit(path: string): Promise<void> {
...
async runAnalyze(
  ...
  try {
    const results = await Promise.all(
      this.plugins.map((p) => this.runAnalyzePlugin(p, files)),
    );
    return results.flat();
  } catch (err) {
...
private async runAnalyzePlugin(
...
async runScore(
  ...
  for (const p of this.plugins) {
    b = await this.applyScorePlugin(p, b, files);
  }
  return b;
}

private async applyScorePlugin(
  plugin: CodeSentinelPlugin,
  breakdown: ScoreBreakdown,
  files: { path: string; content: string }[],
): Promise<ScoreBreakdown> {
