...
export class PluginManager {
  ...
  private async loadPlugin(path: string): Promise<CodeSentinelPlugin | null> {
    try {
      return this.validatePlugin(path, (await import(path)).default);
    } catch (err) {
      this.ctx.logger.warn(`Failed to load plugin "${path}":`, err);
      return null;
    }
  ...
  async runAnalyze(
    ...
    try {
      const results = await Promise.all(
        this.plugins.map((p) => this.runPlugAnalyze(p, files)),
      );
      return results.flat();
    } catch (err) {
  ...
  async runScore(
    ...
    for (const p of this.plugins) {
      b = await this.runPlugScore(p, b, files);
    }
    return b;
  }

  private async runPlugScore(
    p: CodeSentinelPlugin,
    b: ScoreBreakdown,
    files: { path: string; content: string }[],
  ): Promise<ScoreBreakdown> {
