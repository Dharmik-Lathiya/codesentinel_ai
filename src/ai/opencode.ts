...
export function isModelLevelFailure(err: unknown): boolean {
...
export class OpenCodeProvider implements AIProvider {
  ...
  private async completeViaCli(req: CompletionRequest): Promise<CompletionResult> {
    ...
    let lastError: unknown = new Error("no CLI attempt was made");
    for (let i = 0; i < candidates.length; i++) {
      ...
      try {
        ...
        logger.warn(
          `OpenCodeProvider: model ${candidate} not usable on this account (${err instanceof Error ? err.message.slice(0, 140) : String(err)}) — falling back to ${next}`,
        );
      }
    }
    throw lastError;
      ...
      child.on("close", (code) => {
        ...
        if (code !== 0) {
          ...
          if (parsed.content && !isModelLevelFailure(combined)) {
            logger.warn(
              `OpenCode CLI exited with code ${code} but produced ${parsed.content.length} chars of output — salvaging result for ${cliModel}`,
            );
            settled = true;
            resolve({
  ...
  private static parseCliOutput(stdout: string): {
    content: string;
    promptTokens?: number;
