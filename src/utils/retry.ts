...
const RETRY_STATUS_RATE_LIMIT = 429;
const RETRY_STATUS_BAD_GATEWAY = 502;
const RETRY_STATUS_SERVICE_UNAVAILABLE = 503;
const RETRYABLE_STATUS_CODES = new Set([RETRY_STATUS_RATE_LIMIT, RETRY_STATUS_BAD_GATEWAY, RETRY_STATUS_SERVICE_UNAVAILABLE]);
...
const DEFAULT_SHOULD_RETRY = (err: unknown): boolean => {
  ...
  if (err instanceof Error) {
    ...
    return (
      ...
      new RegExp(`\\b${HTTP_STATUS_RATE_LIMIT}\\b`).test(msg) ||
      new RegExp(`\\b${HTTP_STATUS_SERVICE_UNAVAILABLE}\\b`).test(msg) ||
      new RegExp(`\\b${HTTP_STATUS_BAD_GATEWAY}\\b`).test(msg) ||
