...
const MAX_BUFFER = BUFFER_MEGABYTES * BYTES_PER_MEGABYTE;
...
export async function git(
  ...
  try {
    ...
    if (!options.quiet) {
      logger.error(gitErrorMessage(timedOut, command), err);
    }
    throw err;
  }
}
function gitErrorMessage(timedOut: boolean, command: string): string {
  return timedOut
...
export async function collectDiff(
  ...
  try {
    ...
    logger.debug("Failed to resolve a default base ref; falling back to working tree diff");
    baseRef = undefined;
  }
