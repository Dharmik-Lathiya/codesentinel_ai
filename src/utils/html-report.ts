...
export function renderHtmlReport(report: EngineReport): string {
  ...
  return renderReportDocument(report, severityCounts, severityChart, categoryChart, findingsRows, fixRows, testRows);
}
...
<div class="container">
  ...
  ${renderReviewSummary(report.summary)}
  ...
  ${renderCommentsTable(report.comments)}
...
function renderReviewSummary(summary: string): string {
  if (!summary) return "";
  return `<h2>Summary</h2>\n  <p class="meta">${escapeHtml(summary)}</p>`;
}

function renderCommentsTable(comments: EngineReport["comments"]): string {
  ...
  const rows = comments
    ...
    .join("\n");
  return `<h2>AI Review Comments</h2>
  <table>
