...
export function renderHtmlReport(report: EngineReport): string {
  const { categoryCounts, severityCounts } = buildCounts(report);
  const charts = renderReportCharts(categoryCounts, severityCounts);
  const { findingsRows, fixRows, testRows } = buildReportRows(report);
...
function buildCounts(report: EngineReport): { categoryCounts: Record<string, number>; severityCounts: Record<string, number> } {
...
function renderReportCharts(categoryCounts: Record<string, number>, severityCounts: Record<string, number>): string {
  ...
  return `${severityChart}${severityChart && categoryChart ? "\n\n  " : ""}${categoryChart}`;
}

function buildReportRows(report: EngineReport): { findingsRows: string; fixRows: string; testRows: string } {
  ...
  return { findingsRows, fixRows, testRows };
}
