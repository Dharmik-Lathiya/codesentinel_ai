...
<div class="container">
  ...
  <h2>Findings</h2>
  ${renderFindingsTable(findingsRows)}

  ${renderFixTable(fixRows)}

  ${renderTestsTable(testRows)}
...
function renderSummaryCards(report: EngineReport, severityCounts: Record<string, number>): string {
  const parts: string[] = SEVERITY_ORDER.filter((s) => severityCounts[s] != null).map((s) => `${severityCounts[s]} ${escapeHtml(s)}`);
  for (const [s, c] of Object.entries(severityCounts)) {
    if (!SEVERITY_ORDER.includes(s)) parts.push(`${c} ${escapeHtml(s)}`);
  }
  const severitySummary = parts.join(", ") || "none";
  return `
  <div class="cards">
    <div class="card">
      <div class="label">Findings</div>
      <div class="value">${report.findings.length}</div>
      <div class="sub">${severitySummary}</div>
    </div>
...
function renderFindingsTable(rows: string): string {
  if (rows === "") return `<div class="empty">No findings detected.</div>`;
  return `<table>
...
function renderFixTable(rows: string): string {
  if (rows === "") return "";
...
function renderTestsTable(rows: string): string {
  if (rows === "") return "";
