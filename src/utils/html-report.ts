...
const REPORT_STYLES = `  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bar { display: flex; flex-direction: column; align-items: center; flex: 1; height: 120px; }
...
export function renderHtmlReport(report: EngineReport): string {
  ...
  for (const f of report.findings) {
    categoryCounts[f.category ?? "other"] = (categoryCounts[f.category ?? "other"] ?? 0) + 1;
    severityCounts[f.severity ?? "unknown"] = (severityCounts[f.severity ?? "unknown"] ?? 0) + 1;
  }
