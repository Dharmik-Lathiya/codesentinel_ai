...
export function renderHtmlReport(report: EngineReport): string {
  ...
  const severityCounts = report.metrics.findingsBySeverity ?? tallySeverities(report.findings);
...
function tallySeverities(findings: EngineReport["findings"]): Record<string, number> {
