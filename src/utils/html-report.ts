...
const REPORT_STYLES = `  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    table { width: ${TABLE_WIDTH_PERCENT}; border-collapse: collapse; background: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px rgba(0,0,0,${TABLE_SHADOW_ALPHA}); margin-bottom: 1.5rem; }
...
export function renderHtmlReport(report: EngineReport): string {
  const { categoryCounts, severityCounts } = summarizeFindings(report.findings);

  const findingsRows = buildFindingsRows(report.findings);
  const fixRows = buildFixRows(report.fixAttempts);
  const testRows = buildTestRows(report.generatedTests);
...
function summarizeFindings(findings: EngineReport["findings"]): {
...
function buildTestRows(generatedTests: EngineReport["generatedTests"]): string {
