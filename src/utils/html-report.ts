...
<div class="container">
  ...
  ${renderFixTable(report.fixAttempts)}

  ${renderTestsTable(report.generatedTests)}
...
function renderFindingsTable(findings: EngineReport["findings"]): string {
  if (findings.length === 0) return `<div class="empty">No findings detected.</div>`;
  const rows = findings
    .map((f) => {
...
function renderFixTable(fixAttempts: EngineReport["fixAttempts"]): string {
  if (fixAttempts.length === 0) return "";
  const rows = fixAttempts
    .map((a) => {
...
function renderTestsTable(generatedTests: EngineReport["generatedTests"]): string {
  if (generatedTests.length === 0) return "";
  const rows = generatedTests
