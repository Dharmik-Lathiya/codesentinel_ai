...
export function renderHtmlReport(report: EngineReport): string {
  ...
  const metricSeverity = report.metrics.findingsBySeverity;
  if (Object.keys(metricSeverity).length > 0) {
    for (const [key, value] of Object.entries(metricSeverity)) severityCounts[key] = value;
  } else {
    for (const f of report.findings) {
  ...
  const severityEntries = Object.entries(severityCounts).sort(
    (a, b) => severityRank(a[0]) - severityRank(b[0]) || b[1] - a[1],
  );
  const categoryEntries = Object.entries(categoryCounts).sort((a, b) => b[1] - a[1]);

  const severityChart = renderBarChart(
    "Severity Distribution",
    severityEntries.map(([s, c]) => ({ key: s, value: c, color: SEVERITY_COLORS[s] ?? "#6b7280" })),
  );
  const categoryChart = renderBarChart(
    "Category Breakdown",
    categoryEntries.map(([c, n]) => ({ key: c, value: n, color: "#6366f1" })),
  );
...
<div class="container">
  ...
  <div class="cards">
    <div class="card">
      ...
      <div class="sub">${severityEntries.map(([s, c]) => `${c} ${escapeHtml(s)}`).join(", ") || "none"}</div>
    </div>
