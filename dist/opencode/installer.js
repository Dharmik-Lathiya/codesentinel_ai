...
export async function setupOpenCode(version) {
    ...
    try {
        ...
        if (!asset)
            throw new Error(`No binary asset found for ${binaryName}`);
...
export function runOpenCode(binaryPath, args, opts) {
    const env = {
        ...process.env,
        ...(opts?.env ?? {}),
        OPENCODE_CLI: "true",
    };
    const allowedKeys = ["OPENCODE_API_KEY", "OPENCODE_BASE_URL", "GITHUB_TOKEN", "OPENCODE_CLI", "PATH", "HOME", "NODE_ENV"];
    const sandboxed = {};
...
function findAsset(release, binaryName) {
...
async function verifyChecksum(buffer, release, assetName) {
    ...
    if (actual.toLowerCase() !== expected.toLowerCase()) {
        throw new Error(`OpenCode: checksum mismatch for ${assetName} — expected ${expected}, got ${actual}. ` +
