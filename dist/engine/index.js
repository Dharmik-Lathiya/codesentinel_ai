...
import { TestGenerator } from "../testgen/index.js";
import { collectDiff } from "../utils/git.js";
...
import { parseJsonlString, validateAndNormalize } from "../jsonl-parser.js";
...
export class Engine {
    ...
    aiOverride) {
        ...
        this.analyzer = new StaticAnalyzer(config.analyzer, resolve(root, config.cache_dir, "analysis"));
        ...
        if (config.mcp.enabled) {
            this.mcp = new MCPManager(config.mcp.servers.length ? config.mcp.servers : getDefaultMCPServers());
        }
    ...
    async run() {
        ...
        report.metrics.durationMs = Date.now() - start;
        if (this.config.output.writeReportFile)
    ...
    async collectedFiles() {
        if (this.config.mode === "review" || this.config.mode === "fix") {
            const diffs = await collectDiff(undefined, this.root);
            if (diffs.length > 0) {
                return diffs
    ...
    async analyzeFiles(files) {
        const allFindings = [];
        const linterResults = this.config.linters.enabled
            ? runLinters(this.root, { tools: this.config.linters.tools, args: this.config.linters.args })
            : [];
        const scannerResults = this.config.enableSecretScanner
        ...
        for (const file of files) {
            ...
            const cached = this.config.enable_cache
                ? this.cache.get("static", cacheKey)
                : null;
            if (cached) {
    ...
    async recordPatterns(findings) {
        if (!this.learning || !this.config.learning.patternDiscovery)
            return;
        try {
...
- JSON format: { "fixed": bool, "explanation": "...", "hunks": [...] }`;
    ...
    async computeScore(files, findings) {
        const baseline = this.scorer.scoreStatic(files.map((f) => ({ path: f.path, content: f.content })), findings);
        ...
        try {
            const cached = this.config.enable_cache
                ? this.cache.get("score", cacheKey)
                : null;
            const ai = cached ?? (await this.callScoreAI(files));
            if (!cached && this.config.enable_cache)
                this.cache.set("score", cacheKey, ai);
    ...
    async generatePlan(title, description) {
        if (!this.aiAvailable)
            return "AI provider not reachable.";
    ...
    buildSummary(mode, findings, fixAttempts, aiSummaries) {
        ...
        if (issues.length > 0) {
            ...
            const reasons = top.map((i) => `${i.file}${i.line ? `:${i.line}` : ""} — ${i.comment}`);
            parts.push(`\n**Reasoning:** ${reasons.join("; ")}`);
        }
        if (strengths.length > 0) {
            parts.push(`\n### Strengths\n`);
            for (const s of strengths) {
                parts.push(`- **${s.file}${s.line ? `:${s.line}` : ""}** — ${s.comment}`);
            }
        }
        if (issues.length > 0) {
            ...
            for (const i of visible) {
                const label = i.severity === "critical" || i.severity === "high"
                    ...
                    : "";
                parts.push(`- ${label}**${i.file}${i.line ? `:${i.line}` : ""}** — ${i.comment}${i.suggestion ? `\n  > Suggestion: ${i.suggestion}` : ""}`);
            }
            if (hidden > 0) {
        ...
        if (fixAttempts && fixAttempts.length > 0) {
            ...
            for (const a of show) {
                parts.push(`- ${a.fixed ? "✅" : "❌"} **${a.file}** — ${a.explanation}`);
            }
            if (hidden > 0) {
    ...
    writeReportFile(report) {
        ...
        const name = report.mode === "score" && report.score
            ? "score.json"
