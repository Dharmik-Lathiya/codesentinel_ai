...
export class ProgressiveAnalyzer {
    ...
    async analyzeProgressive(files, analyzer) {
        ...
        let totalFindings = [];
        // Quick scan
        const quickResult = await this.performAnalysis(files, analyzer, "quick", this.config.quickScanRules);
        ...
        if (this.config.autoEscalate &&
            ...
            // Standard scan
            const standardResult = await this.performAnalysis(files, analyzer, "standard", this.config.standardScanRules);
            ...
            if (standardResult.findings.length >= this.config.escalationThreshold) {
                const deepResult = await this.performAnalysis(files, analyzer, "deep", this.config.deepScanRules);
            ...
            for (let j = i + 1; j < files.length; j++) {
                ...
                for (let start1 = 0; start1 < lines1.length - minLines; start1++) {
                    for (let start2 = 0; start2 < lines2.length - minLines; start2++) {
                        let matchLength = 0;
                        while (start1 + matchLength < lines1.length &&
